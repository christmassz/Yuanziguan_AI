...
const config = {
  dataDir: __dirname,
  mainCsvFile: path.join(__dirname, 'pi_top-ratio-data.csv'),
  mainJsonFile: path.join(__dirname, 'pi_top-ratio-data.json'),
  rawDataFile: path.join(__dirname, 'pi_top-data.json'),
...
function updateMainDataFile() {
  try {
    ...
    if (!fs.existsSync(config.mainCsvFile)) {
      ...
      const headers = 'date,price,pi_topRatio\n';
    ...
    const csvLines = newEntries.map(item => 
      `${item.date},${item.price.toFixed(2)},${item.pi_topRatio.toFixed(4)}`
    ).join('\n');
...
function cleanupFiles() {
  try {
    ...
    const filesToDelete = [
      ...
      'pi_top-ratio.json', // Temporary JSON from previous steps
      'corrected-pi_top-ratio.json', // Intermediate JSON 
      'corrected-pi_top-ratio.csv',  // Intermediate CSV
...
async function main() {
  try {
    log('Starting pi_top Ratio scraper');
    ...
    await runScript(path.join(config.dataDir, 'convert_pi_top_to_csv.js'));
    log('Converted pi_top data to readable CSV format');
    
    // Step 4: Clean up temporary files
    cleanupFiles();
    
    log('pi_top Ratio scraper completed successfully');
  } catch (error) {
    log(`Error in pi_top Ratio scraper: ${error.message}`);
  } 
}
