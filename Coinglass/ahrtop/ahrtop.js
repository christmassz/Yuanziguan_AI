...
const config = {
  dataDir: __dirname,
  mainCsvFile: path.join(__dirname, 'ahrtop-ratio-data.csv'),
  mainJsonFile: path.join(__dirname, 'ahrtop-ratio-data.json'),
  rawDataFile: path.join(__dirname, 'ahrtop-data.json'),
...
function updateMainDataFile() {
  try {
    ...
    if (!fs.existsSync(config.mainCsvFile)) {
      ...
      const headers = 'date,price,ahrtopRatio\n';
    ...
    const csvLines = newEntries.map(item => 
      `${item.date},${item.price.toFixed(2)},${item.ahrtopRatio.toFixed(4)}`
    ).join('\n');
...
function cleanupFiles() {
  try {
    ...
    const filesToDelete = [
      ...
      'ahrtop-ratio.json', // Temporary JSON from previous steps
      'corrected-ahrtop-ratio.json', // Intermediate JSON 
      'corrected-ahrtop-ratio.csv',  // Intermediate CSV
...
async function main() {
  try {
    log('Starting ahrtop Ratio scraper');
    ...
    await runScript(path.join(config.dataDir, 'convert_ahrtop_to_csv.js'));
    log('Converted ahrtop data to readable CSV format');
    
    // Step 4: Clean up temporary files
    cleanupFiles();
    
    log('ahrtop Ratio scraper completed successfully');
  } catch (error) {
    log(`Error in ahrtop Ratio scraper: ${error.message}`);
  } 
}
