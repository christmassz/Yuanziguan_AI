// mayer_multiple.js - Updated version
const fs = require('fs');
...
const config = {
  dataDir: __dirname,
  mainCsvFile: path.join(__dirname, 'mayer-multiple-data.csv'),
  mainJsonFile: path.join(__dirname, 'mayer-multiple-data.json'),
...
function updateMainDataFile() {
  try {
    // Make sure we have the simplified data
    const simplifiedJsonPath = path.join(config.dataDir, 'simplified-mayer-multiple.json');
    if (!fs.existsSync(simplifiedJsonPath)) {
      log('Simplified data file not found');
      return false;
    }
    
    // Read the new data
    const newData = JSON.parse(fs.readFileSync(simplifiedJsonPath, 'utf8'));
    log(`Read ${newData.length} entries from simplified data`);
    ...
    if (!fs.existsSync(config.mainCsvFile)) {
      ...
      const headers = 'date,price,sma200d,mayerMultiple\n';
...
function cleanupFiles() {
  try {
    ...
    const filesToDelete = [
      ...
      'mayer-multiple.json', // Temporary JSON from previous steps
      'corrected-mayer-multiple.json', // Intermediate JSON 
      'corrected-mayer-multiple.csv',  // Intermediate CSV
      // Keep the simplified files as they're used for updates
    ];
...
async function main() {
  try {
    log('Starting Mayer Multiple scraper');
    
    // Step 1: Run auto-decrypt.js to get the encrypted data and save it to a file
    await runScript(path.join(config.dataDir, 'auto-decrypt.js'));
    
    // Step 2: Run calculate_mayer_multiple.js to process the data
    await runScript(path.join(config.dataDir, 'calculate_mayer_multiple.js'));
    
    // Step 3: Update the main data file with new entries
    updateMainDataFile();
    
    // Step 4: Clean up temporary files
    cleanupFiles();
    
    log('Mayer Multiple scraper completed successfully');
  } catch (error) {
    log(`Error in Mayer Multiple scraper: ${error.message}`);
  } 
}
