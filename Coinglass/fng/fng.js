...
const config = {
  dataDir: __dirname,
  mainCsvFile: path.join(__dirname, 'fng-ratio-data.csv'),
  mainJsonFile: path.join(__dirname, 'fng-ratio-data.json'),
  rawDataFile: path.join(__dirname, 'fng-data.json'),
...
function updateMainDataFile() {
  try {
    ...
    if (!fs.existsSync(config.mainCsvFile)) {
      ...
      const headers = 'date,price,fngRatio\n';
    ...
    const csvLines = newEntries.map(item => 
      `${item.date},${item.price.toFixed(2)},${item.fngRatio.toFixed(4)}`
    ).join('\n');
...
function cleanupFiles() {
  try {
    ...
    const filesToDelete = [
      ...
      'fng-ratio.json', // Temporary JSON from previous steps
      'corrected-fng-ratio.json', // Intermediate JSON 
      'corrected-fng-ratio.csv',  // Intermediate CSV
...
async function main() {
  try {
    log('Starting fng Ratio scraper');
    ...
    await runScript(path.join(config.dataDir, 'convert_fng_to_csv.js'));
    log('Converted fng data to readable CSV format');
    
    // Step 4: Clean up temporary files
    cleanupFiles();
    
    log('fng Ratio scraper completed successfully');
  } catch (error) {
    log(`Error in fng Ratio scraper: ${error.message}`);
  } 
}
