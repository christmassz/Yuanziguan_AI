...
const config = {
  dataDir: __dirname,
  mainCsvFile: path.join(__dirname, 'sth-ratio-data.csv'),
  mainJsonFile: path.join(__dirname, 'sth-ratio-data.json'),
  rawDataFile: path.join(__dirname, 'sth-data.json'),
...
function updateMainDataFile() {
  try {
    ...
    if (!fs.existsSync(config.mainCsvFile)) {
      ...
      const headers = 'date,price,sthRatio\n';
    ...
    const csvLines = newEntries.map(item => 
      `${item.date},${item.price.toFixed(2)},${item.sthRatio.toFixed(4)}`
    ).join('\n');
...
function cleanupFiles() {
  try {
    ...
    const filesToDelete = [
      ...
      'sth-ratio.json', // Temporary JSON from previous steps
      'corrected-sth-ratio.json', // Intermediate JSON 
      'corrected-sth-ratio.csv',  // Intermediate CSV
...
async function main() {
  try {
    log('Starting sth Ratio scraper');
    ...
    await runScript(path.join(config.dataDir, 'convert_sth_to_csv.js'));
    log('Converted sth data to readable CSV format');
    
    // Step 4: Clean up temporary files
    cleanupFiles();
    
    log('sth Ratio scraper completed successfully');
  } catch (error) {
    log(`Error in sth Ratio scraper: ${error.message}`);
  } 
}
