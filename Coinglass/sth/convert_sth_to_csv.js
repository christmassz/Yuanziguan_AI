...
// File paths
const jsonFilePath = path.join(__dirname, 'sth-data.json');
const csvFilePath = path.join(__dirname, 'sth-data-readable.csv');
...
try {
    ...
    let csvContent = 'Date,Price,STH Supply,Log Price,Log Supply,Normalized Ratio,STH Supply Percentage\n';
    ...
    data.forEach((item, i) => {
        ...
        // Calculate normalized ratio using the same approach as LTH
        // Negative values: price is higher than historically expected relative to supply
        // Positive values: supply is higher than historically expected relative to supply
        const normalizedRatio = (logSupply - logPrice) - averageLogRatio;
        
        // Also include the original supplyRatio as a percentage for reference
        const sthSupplyPercentage = (item.supplyRatio * 100).toFixed(2) + '%';
        ...
        // Add to CSV content
        csvContent += `${dateStr},${item.price},${item.supply},${logPrice},${logSupply},${normalizedRatio.toFixed(4)},${sthSupplyPercentage}\n`;
    });
