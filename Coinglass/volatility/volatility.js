...
const config = {
  dataDir: __dirname,
  mainCsvFile: path.join(__dirname, 'volatility-ratio-data.csv'),
  mainJsonFile: path.join(__dirname, 'volatility-ratio-data.json'),
  rawDataFile: path.join(__dirname, 'volatility-data.json'),
...
function updateMainDataFile() {
  try {
    ...
    if (!fs.existsSync(config.mainCsvFile)) {
      ...
      const headers = 'date,price,volatilityRatio\n';
    ...
    const csvLines = newEntries.map(item => 
      `${item.date},${item.price.toFixed(2)},${item.volatilityRatio.toFixed(4)}`
    ).join('\n');
...
function cleanupFiles() {
  try {
    ...
    const filesToDelete = [
      ...
      'volatility-ratio.json', // Temporary JSON from previous steps
      'corrected-volatility-ratio.json', // Intermediate JSON 
      'corrected-volatility-ratio.csv',  // Intermediate CSV
...
async function main() {
  try {
    log('Starting volatility Ratio scraper');
    ...
    await runScript(path.join(config.dataDir, 'convert_volatility_to_csv.js'));
    log('Converted volatility data to readable CSV format');
    
    // Step 4: Clean up temporary files
    cleanupFiles();
    
    log('volatility Ratio scraper completed successfully');
  } catch (error) {
    log(`Error in volatility Ratio scraper: ${error.message}`);
  } 
}
