...
const config = {
  dataDir: __dirname,
  mainCsvFile: path.join(__dirname, 'lth-ratio-data.csv'),
  mainJsonFile: path.join(__dirname, 'lth-ratio-data.json'),
  rawDataFile: path.join(__dirname, 'lth-data.json'),
...
function updateMainDataFile() {
  try {
    ...
    if (!fs.existsSync(config.mainCsvFile)) {
      ...
      const headers = 'date,price,lthRatio\n';
    ...
    const csvLines = newEntries.map(item => 
      `${item.date},${item.price.toFixed(2)},${item.lthRatio.toFixed(4)}`
    ).join('\n');
...
function cleanupFiles() {
  try {
    ...
    const filesToDelete = [
      ...
      'lth-ratio.json', // Temporary JSON from previous steps
      'corrected-lth-ratio.json', // Intermediate JSON 
      'corrected-lth-ratio.csv',  // Intermediate CSV
...
async function main() {
  try {
    log('Starting lth Ratio scraper');
    ...
    await runScript(path.join(config.dataDir, 'convert_lth_to_csv.js'));
    log('Converted lth data to readable CSV format');
    
    // Step 4: Clean up temporary files
    cleanupFiles();
    
    log('lth Ratio scraper completed successfully');
  } catch (error) {
    log(`Error in lth Ratio scraper: ${error.message}`);
  } 
}
