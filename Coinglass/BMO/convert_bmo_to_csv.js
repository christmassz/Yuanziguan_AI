...
// File paths
const jsonFilePath = path.join(__dirname, 'bmo-data.json');
const csvFilePath = path.join(__dirname, 'bmo-data-readable.csv');
...
try {
  ...
  data.forEach((item, i) => {
    ...
    // Get BMO index from the correct field
    const bmoIndex = item.index !== undefined ? item.index : 'Unknown';
    
    // Add to CSV
    csvContent += `${dateStr},${bmoIndex}\n`;
  });
