...
const config = {
  dataDir: __dirname,
  mainCsvFile: path.join(__dirname, 'bmo-ratio-data.csv'),
  mainJsonFile: path.join(__dirname, 'bmo-ratio-data.json'),
  rawDataFile: path.join(__dirname, 'bmo-data.json'),
...
function updateMainDataFile() {
  try {
    ...
    if (!fs.existsSync(config.mainCsvFile)) {
      ...
      const headers = 'date,price,bmoRatio\n';
    ...
    const csvLines = newEntries.map(item => 
      `${item.date},${item.price.toFixed(2)},${item.bmoRatio.toFixed(4)}`
    ).join('\n');
...
function cleanupFiles() {
  try {
    ...
    const filesToDelete = [
      ...
      'bmo-ratio.json', // Temporary JSON from previous steps
      'corrected-bmo-ratio.json', // Intermediate JSON 
      'corrected-bmo-ratio.csv',  // Intermediate CSV
...
async function main() {
  try {
    log('Starting bmo Ratio scraper');
    ...
    await runScript(path.join(config.dataDir, 'convert_bmo_to_csv.js'));
    log('Converted bmo data to readable CSV format');
    
    // Step 4: Clean up temporary files
    cleanupFiles();
    
    log('bmo Ratio scraper completed successfully');
  } catch (error) {
    log(`Error in bmo Ratio scraper: ${error.message}`);
  } 
}
