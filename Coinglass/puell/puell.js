...
const config = {
  dataDir: __dirname,
  mainCsvFile: path.join(__dirname, 'puell-ratio-data.csv'),
  mainJsonFile: path.join(__dirname, 'puell-ratio-data.json'),
  rawDataFile: path.join(__dirname, 'puell-data.json'),
...
function updateMainDataFile() {
  try {
    ...
    if (!fs.existsSync(config.mainCsvFile)) {
      ...
      const headers = 'date,price,puellRatio\n';
    ...
    const csvLines = newEntries.map(item => 
      `${item.date},${item.price.toFixed(2)},${item.puellRatio.toFixed(4)}`
    ).join('\n');
...
function cleanupFiles() {
  try {
    ...
    const filesToDelete = [
      ...
      'puell-ratio.json', // Temporary JSON from previous steps
      'corrected-puell-ratio.json', // Intermediate JSON 
      'corrected-puell-ratio.csv',  // Intermediate CSV
...
async function main() {
  try {
    log('Starting puell Ratio scraper');
    ...
    await runScript(path.join(config.dataDir, 'convert_puell_to_csv.js'));
    log('Converted puell data to readable CSV format');
    
    // Step 4: Clean up temporary files
    cleanupFiles();
    
    log('puell Ratio scraper completed successfully');
  } catch (error) {
    log(`Error in puell Ratio scraper: ${error.message}`);
  } 
}
