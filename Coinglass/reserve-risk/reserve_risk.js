...
const config = {
  dataDir: __dirname,
  mainCsvFile: path.join(__dirname, 'reserve_risk-ratio-data.csv'),
  mainJsonFile: path.join(__dirname, 'reserve_risk-ratio-data.json'),
  rawDataFile: path.join(__dirname, 'reserve_risk-data.json'),
...
function updateMainDataFile() {
  try {
    ...
    if (!fs.existsSync(config.mainCsvFile)) {
      ...
      const headers = 'date,price,reserve_riskRatio\n';
    ...
    const csvLines = newEntries.map(item => 
      `${item.date},${item.price.toFixed(2)},${item.reserve_riskRatio.toFixed(4)}`
    ).join('\n');
...
function cleanupFiles() {
  try {
    ...
    const filesToDelete = [
      ...
      'reserve_risk-ratio.json', // Temporary JSON from previous steps
      'corrected-reserve_risk-ratio.json', // Intermediate JSON 
      'corrected-reserve_risk-ratio.csv',  // Intermediate CSV
...
async function main() {
  try {
    log('Starting reserve_risk Ratio scraper');
    ...
    await runScript(path.join(config.dataDir, 'convert_reserve_risk_to_csv.js'));
    log('Converted reserve_risk data to readable CSV format');
    
    // Step 4: Clean up temporary files
    cleanupFiles();
    
    log('reserve_risk Ratio scraper completed successfully');
  } catch (error) {
    log(`Error in reserve_risk Ratio scraper: ${error.message}`);
  } 
}
