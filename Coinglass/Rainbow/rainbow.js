...
const config = {
  dataDir: __dirname,
  mainCsvFile: path.join(__dirname, 'rainbow-ratio-data.csv'),
  mainJsonFile: path.join(__dirname, 'rainbow-ratio-data.json'),
  rawDataFile: path.join(__dirname, 'rainbow-data.json'),
...
function updateMainDataFile() {
  try {
    ...
    if (!fs.existsSync(config.mainCsvFile)) {
      ...
      const headers = 'date,price,rainbowRatio\n';
    ...
    const csvLines = newEntries.map(item => 
      `${item.date},${item.price.toFixed(2)},${item.rainbowRatio.toFixed(4)}`
    ).join('\n');
...
function cleanupFiles() {
  try {
    ...
    const filesToDelete = [
      ...
      'rainbow-ratio.json', // Temporary JSON from previous steps
      'corrected-rainbow-ratio.json', // Intermediate JSON 
      'corrected-rainbow-ratio.csv',  // Intermediate CSV
...
async function main() {
  try {
    log('Starting rainbow Ratio scraper');
    ...
    await runScript(path.join(config.dataDir, 'convert_rainbow_to_csv.js'));
    log('Converted rainbow data to readable CSV format');
    
    // Step 4: Clean up temporary files
    cleanupFiles();
    
    log('rainbow Ratio scraper completed successfully');
  } catch (error) {
    log(`Error in rainbow Ratio scraper: ${error.message}`);
  } 
}
