...
const config = {
  dataDir: __dirname,
  mainCsvFile: path.join(__dirname, 'mzrv-z-ratio-data.csv'),
  mainJsonFile: path.join(__dirname, 'mzrv-z-ratio-data.json'),
  rawDataFile: path.join(__dirname, 'mzrv-z-data.json'),
...
function updateMainDataFile() {
  try {
    ...
    if (!fs.existsSync(config.mainCsvFile)) {
      ...
      const headers = 'date,price,mzrv-zRatio\n';
    ...
    const csvLines = newEntries.map(item => 
      `${item.date},${item.price.toFixed(2)},${item.mzrv-zRatio.toFixed(4)}`
    ).join('\n');
...
function cleanupFiles() {
  try {
    ...
    const filesToDelete = [
      ...
      'mzrv-z-ratio.json', // Temporary JSON from previous steps
      'corrected-mzrv-z-ratio.json', // Intermediate JSON 
      'corrected-mzrv-z-ratio.csv',  // Intermediate CSV
...
async function main() {
  try {
    log('Starting mzrv-z Ratio scraper');
    ...
    await runScript(path.join(config.dataDir, 'convert_mzrv-z_to_csv.js'));
    log('Converted mzrv-z data to readable CSV format');
    
    // Step 4: Clean up temporary files
    cleanupFiles();
    
    log('mzrv-z Ratio scraper completed successfully');
  } catch (error) {
    log(`Error in mzrv-z Ratio scraper: ${error.message}`);
  } 
}
