...
const config = {
  dataDir: __dirname,
  mainCsvFile: path.join(__dirname, 'rhodl-ratio-data.csv'),
  mainJsonFile: path.join(__dirname, 'rhodl-ratio-data.json'),
  rawDataFile: path.join(__dirname, 'rhodl-data.json'),
...
function updateMainDataFile() {
  try {
    ...
    if (!fs.existsSync(config.mainCsvFile)) {
      ...
      const headers = 'date,price,rhodlRatio\n';
    ...
    const csvLines = newEntries.map(item => 
      `${item.date},${item.price.toFixed(2)},${item.rhodlRatio.toFixed(4)}`
    ).join('\n');
...
function cleanupFiles() {
  try {
    ...
    const filesToDelete = [
      ...
      'rhodl-ratio.json', // Temporary JSON from previous steps
      'corrected-rhodl-ratio.json', // Intermediate JSON 
      'corrected-rhodl-ratio.csv',  // Intermediate CSV
...
async function main() {
  try {
    log('Starting rhodl Ratio scraper');
    ...
    await runScript(path.join(config.dataDir, 'convert_rhodl_to_csv.js'));
    log('Converted rhodl data to readable CSV format');
    
    // Step 4: Clean up temporary files
    cleanupFiles();
    
    log('rhodl Ratio scraper completed successfully');
  } catch (error) {
    log(`Error in rhodl Ratio scraper: ${error.message}`);
  } 
}
