...
// File paths
const jsonFilePath = path.join(__dirname, 'rhodl-data.json');
const csvFilePath = path.join(__dirname, 'rhodl-data-readable.csv');
...
try {
  ...
  let csvContent = 'Date,rhodl Index\n';
  ...
  data.forEach((item, i) => {
    ...
    // Get rhodl index - it's called 'index' in the data, not 'escapeIndex'
    const rhodlIndex = item.index !== undefined ? item.index : 'Unknown';
    
    // Add to CSV
    csvContent += `${dateStr},${rhodlIndex}\n`;
  });
