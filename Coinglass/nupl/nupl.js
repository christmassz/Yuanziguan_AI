...
const config = {
  dataDir: __dirname,
  mainCsvFile: path.join(__dirname, 'nupl-ratio-data.csv'),
  mainJsonFile: path.join(__dirname, 'nupl-ratio-data.json'),
  rawDataFile: path.join(__dirname, 'nupl-data.json'),
...
function updateMainDataFile() {
  try {
    ...
    if (!fs.existsSync(config.mainCsvFile)) {
      ...
      const headers = 'date,price,nuplRatio\n';
    ...
    const csvLines = newEntries.map(item => 
      `${item.date},${item.price.toFixed(2)},${item.nuplRatio.toFixed(4)}`
    ).join('\n');
...
function cleanupFiles() {
  try {
    ...
    const filesToDelete = [
      ...
      'nupl-ratio.json', // Temporary JSON from previous steps
      'corrected-nupl-ratio.json', // Intermediate JSON 
      'corrected-nupl-ratio.csv',  // Intermediate CSV
...
async function main() {
  try {
    log('Starting nupl Ratio scraper');
    ...
    await runScript(path.join(config.dataDir, 'convert_nupl_to_csv.js'));
    log('Converted nupl data to readable CSV format');
    
    // Step 4: Clean up temporary files
    cleanupFiles();
    
    log('nupl Ratio scraper completed successfully');
  } catch (error) {
    log(`Error in nupl Ratio scraper: ${error.message}`);
  } 
}
