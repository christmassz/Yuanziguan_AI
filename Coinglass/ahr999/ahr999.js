...
const config = {
  dataDir: __dirname,
  mainCsvFile: path.join(__dirname, 'ahr999-ratio-data.csv'),
  mainJsonFile: path.join(__dirname, 'ahr999-ratio-data.json'),
  rawDataFile: path.join(__dirname, 'ahr999-data.json'),
...
function updateMainDataFile() {
  try {
    ...
    if (!fs.existsSync(config.mainCsvFile)) {
      ...
      const headers = 'date,price,ahr999Ratio\n';
    ...
    const csvLines = newEntries.map(item => 
      `${item.date},${item.price.toFixed(2)},${item.ahr999Ratio.toFixed(4)}`
    ).join('\n');
...
function cleanupFiles() {
  try {
    ...
    const filesToDelete = [
      ...
      'ahr999-ratio.json', // Temporary JSON from previous steps
      'corrected-ahr999-ratio.json', // Intermediate JSON 
      'corrected-ahr999-ratio.csv',  // Intermediate CSV
...
async function main() {
  try {
    log('Starting ahr999 Ratio scraper');
    ...
    await runScript(path.join(config.dataDir, 'convert_ahr999_to_csv.js'));
    log('Converted ahr999 data to readable CSV format');
    
    // Step 4: Clean up temporary files
    cleanupFiles();
    
    log('ahr999 Ratio scraper completed successfully');
  } catch (error) {
    log(`Error in ahr999 Ratio scraper: ${error.message}`);
  } 
}
