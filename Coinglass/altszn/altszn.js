...
const config = {
  dataDir: __dirname,
  mainCsvFile: path.join(__dirname, 'altszn-ratio-data.csv'),
  mainJsonFile: path.join(__dirname, 'altszn-ratio-data.json'),
  rawDataFile: path.join(__dirname, 'altszn-data.json'),
...
function updateMainDataFile() {
  try {
    ...
    if (!fs.existsSync(config.mainCsvFile)) {
      ...
      const headers = 'date,price,altsznRatio\n';
    ...
    const csvLines = newEntries.map(item => 
      `${item.date},${item.price.toFixed(2)},${item.altsznRatio.toFixed(4)}`
    ).join('\n');
...
function cleanupFiles() {
  try {
    ...
    const filesToDelete = [
      ...
      'altszn-ratio.json', // Temporary JSON from previous steps
      'corrected-altszn-ratio.json', // Intermediate JSON 
      'corrected-altszn-ratio.csv',  // Intermediate CSV
...
async function main() {
  try {
    log('Starting altszn Ratio scraper');
    ...
    await runScript(path.join(config.dataDir, 'convert_altszn_to_csv.js'));
    log('Converted altszn data to readable CSV format');
    
    // Step 4: Clean up temporary files
    cleanupFiles();
    
    log('altszn Ratio scraper completed successfully');
  } catch (error) {
    log(`Error in altszn Ratio scraper: ${error.message}`);
  } 
}
